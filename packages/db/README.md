...
- `DATABASE_URL` must be present in the root environment for the package's
  `pnpm --filter @agile-tools/db prisma:*` scripts. These scripts invoke the
  Prisma CLI directly and therefore only read `DATABASE_URL` — they do **not**
  run the `DATABASE_URL_ENV_VAR` indirection.
- The `DATABASE_URL_ENV_VAR` indirection (for managed-service deployments that
  inject the PostgreSQL binding under an autogenerated variable name) only
  applies to the app/container runtime, where it is resolved by
  `resolveDatabaseUrlFromEnv()` in `@agile-tools/shared` (invoked by
  `getPrismaClient()` and the production `docker/run.sh` entrypoint). When
  running Prisma CLI commands directly against such a deployment, export
  `DATABASE_URL` manually first, e.g.
  `DATABASE_URL="$XYZ_POSTGRESQL_URI" pnpm --filter @agile-tools/db prisma:migrate`.
